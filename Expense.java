...
class ExpenseManager
{
	...
	{
		int loc = 0;
		Expense [] temp = new Expense[size];
		for(Expense ex : _allExpenses)
		{
			if((ex != null) && (ex.getDetails().contains(detail)))
			{
				temp[loc]=new Expense(ex.getId(), ex.getAmount(), ex.getDate(), ex.getDetails());
				loc++;
			}
...
class FifthExample
{
	...
	private static void deletingExpense()
	{
		int id= MyConsole.getNumber("Enter the id of the expenses that you want to delete from database");
		try
		{
			//Expense  ex = _mgr.deleteExpense(id);
			_mgr.deleteExpense(id);
				//if(_mgr.deleteExpense(id) == null)
					MyConsole.print("Successfully deleted expense....");
	...
	/*
	private static void updatingExpense(){
		int id= MyConsole.getNumber("Enter the id of the expenses that you want to update in database");
		try{
			Expense ex=new Expense(id);
			_mgr.updateExpense(ex);
				//if(_mgr.deleteExpense(id) == null)
					MyConsole.print("Successfully updated expense....");
